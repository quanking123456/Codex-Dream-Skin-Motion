...
- 修复动态主题切换时重复挂载、约 8–10 秒闪烁以及周期性退回海报图的问题；同一运行时 revision 的重复注入保持幂等。
- 新增仅绑定 `127.0.0.1`、使用随机令牌并支持 Range 请求的受限媒体回退服务；默认仍优先使用 CDP 文件句柄。
- 增加 `Wlop` 动态主题和 `R-c` 静态主题的可下载示例目录，并将更新、Issue 与 Release 链接切换到 Motion 分支仓库。
