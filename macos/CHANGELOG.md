...
- 同步共享 renderer 的视频背景层、生命周期与可见性处理，使跨平台生成资产继续保持一致；当前本地 MP4 选择与管理入口由 Windows 托盘提供。
- 将更新、Issue 与 Release 链接切换到 Codex Dream Skin Motion 分支仓库。
- 增加 `Wlop` 动态主题和 `R-c` 静态主题的可下载示例目录；视频主题仍通过 Windows 已保存主题目录安装，不放宽社区 ZIP 的图片-only 安全契约。
