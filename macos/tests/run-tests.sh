...
CURRENT_VERSION="$(/usr/bin/tr -d '[:space:]' < "$ROOT/VERSION")"
...
if /usr/bin/xcrun --sdk macosx --show-sdk-platform-path >/dev/null 2>&1; then
  "$ROOT/scripts/test-menubar-app.sh"
else
...
"$NODE" -e '
  ...
  if (value.releaseUrl !== "https://github.com/quanking123456/Codex-Dream-Skin-Motion/releases/latest") process.exit(1);
' "$UPDATE_JSON" "$CURRENT_VERSION"
...
"$NODE" -e '
  ...
  if (!value.updateAvailable) process.exit(1);
  if (value.releaseUrl !== "https://github.com/quanking123456/Codex-Dream-Skin-Motion/releases/latest") process.exit(1);
' "$UPDATE_REDIRECT_JSON" "$CURRENT_VERSION"
...
/usr/bin/grep -F -q 'assets/portal-hero.png' \
  "$STANDALONE_ROOT/docs/reference-background-prompt-guide.md"
/usr/bin/grep -F -q 'https://github.com/quanking123456/Codex-Dream-Skin-Motion/blob/main/windows/assets/theme.json' \
  "$STANDALONE_ROOT/docs/reference-background-prompt-guide.md"
...
/usr/bin/grep -F -q \
  'https://github.com/quanking123456/Codex-Dream-Skin-Motion/blob/main/windows/assets/theme.json' \
  "$REPACK_GUIDE"
...
/usr/bin/env -u HOME /bin/bash -c \
  '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "$2" ]' \
  _ "$ROOT" "$CURRENT_VERSION"
